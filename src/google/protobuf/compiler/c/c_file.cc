...
FileGenerator::FileGenerator(const FileDescriptor* file,
  ...
  package_name_ = GetPackageName(file_->package());
}
...
void FileGenerator::GenerateHeader(io::Printer* printer) {
  ...
  printer->Print(
    ...
    "\n"
    "#include <google/protobuf-c-ng/protobuf-c.h>\n"
    "\n"
  ...
  printer->Print("extern void pbc_memcpy(void*,const void*,size_t);\n");
  printer->Print(
		  "typedef struct _$packagename$__context $packagename$__context;\n",
  ...
  printer->Print("void $package_name$_context_init($package_name$__context* $package_name$\n","package_name",package_name_);
  
  for (int i = 0; i < file_->dependency_count(); i++) {
    printer->Print(
      ",const $dependpackage$__context* $dependpackage$\n",
      "dependpackage", GetPackageName(file_->dependency(i)->package()));
  }
  printer->Print("\n);\n");

  for (int i = 0; i < file_->message_type_count(); i++) {
    message_generators_[i]->GenerateHelperFunctionDeclarations(printer, package_name_,false);
  }

  //MAOLIN
  // Generate context definition
  printer->Print(
		  "struct _$package_name$__context {\n",
		  "package_name",package_name_);
  ...
  for(int i=0; i < file_->message_type_count(); i++) {
	  message_generators_[i]->GenerateEnumValueDeclarations(printer);
	  message_generators_[i]->GenerateDescriptorValueDeclarations(printer);
	  printer->Print("\n");
  }
  printer->Print("};\n");
...
#if 0			// services & extension NOT supported by MAOLIN :(
  printer->Print("\n/* --- services --- */\n\n");
...
void FileGenerator::GenerateSource(io::Printer* printer) {
  ...
  //MAOLIN Generate context init function
  printer->Print("void $package_name$_context_init($package_name$__context* $package_name$\n","package_name",package_name_);
  ...
  */
  ...
  for (int i = 0; i < file_->message_type_count(); i++) {
    message_generators_[i]->GenerateMessageDescriptorAssign(printer);
  }
  for (int i = 0; i < file_->enum_type_count(); i++) {
  ...
  for (int i = 0; i < file_->message_type_count(); i++) {
    message_generators_[i]->GenerateHelperFunctionDefinitions(printer,package_name_, false);
  }
